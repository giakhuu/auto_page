...
AUTO_SCHEDULE_TIMES = ((8, 0), (10, 0), (14, 00), (16, 0), (17, 0))
...
def get_publish_usage() -> str:
    ...
    return "Usage: /publish <url> or /publish <url> | optional caption"
...
def parse_publish_request(text: str) -> PublishRequest:
    ...
    url_part, separator, caption_part = payload.partition("|")
    source_url = url_part.strip()
    caption = caption_part.strip() if separator else ""

    parsed = urlparse(source_url)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ValueError(f"Invalid URL. {get_publish_usage()}")

    return PublishRequest(source_url=source_url, caption=caption)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await reply_text(
        update,
        "Commands: /start, /help, /publish <url> | optional caption. Empty captions will be copied from the source post when available.",
    )
